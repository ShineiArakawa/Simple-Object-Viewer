...
void Renderer::updateScale(float acScale) {
  _acScaleMat = glm::scale(glm::vec3(acScale, acScale, acScale));
}

void Renderer::updateTranslate(const glm::vec4& newPosScreenSpace,
...
void Renderer::updateRotate(const glm::vec3& u, const glm::vec3& v) {
  const double angle =
      std::acos(std::max(-1.0f, std::min(glm::dot(u, v), 1.0f)));
  const glm::vec3 rotAxis = glm::cross(u, v);
  const glm::mat4 c2wMat = glm::inverse(_viewMat);
  const glm::vec3 rotAxisWorldSpace =
      glm::vec3(c2wMat * glm::vec4(rotAxis, 0.0f));
...
void Renderer::rotateModel(const float angle,
                           const glm::vec3 rotAxisWorldSpace) {

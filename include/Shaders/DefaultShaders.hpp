...
class DefaultShaders {
 public:
  ...
  inline static const std::string FRAG_SHADER =
      ...
      "    }\n"
      "\n"
      "    if(u_toUseTexture > 3.5 && u_toUseTexture < 4.5) {\n"
      "        // Material shading\n"
      "        shading(u_diffuseColor, u_specularColor, u_ambientColor);\n"
      "    } else if(u_toUseTexture > 2.5 && u_toUseTexture < 3.5) {\n"
      "        // Texture with shading\n"
      "        vec3 textureColor = texture(u_texture, f_uv).xyz;\n"
      "        shading(textureColor, u_specularColor, textureColor);\n"
